...
python_download_directory = "liaoxuefeng/python"
javascript_download_directory = "liaoxuefeng/javascript"
git_download_directory = "liaoxuefeng/git"
python_url_path = "wiki/0014316089557264a6b348958f449949df42a6d3a2e542c000"
javascript_url_path = "wiki/001434446689867b27157e896e74d51a89c25cc8b43bdb3000"
git_url_path = "wiki/0013739516305929606dd18361248578c67b8067c8c017b000"
# 网页中引用的css与image资源路径
source_link = set()

# 引用不同的链接，下载不同的内容
download_directory_out = python_download_directory
source_url_path_out = python_url_path
source_link.clear()
bs_obj_out = get_html(source_url_path_out)
...
for html_out in html_list_out:
    ...
    download_css(download_directory_out, bs_obj_new_out)
    download_page_img(download_directory_out, bs_obj_new_out)
    download_page(download_directory_out, html_out.url, bs_obj_new_out, html_list_out)

# 引用不同的链接，下载不同的内容
download_directory_out = javascript_download_directory
source_url_path_out = javascript_url_path
source_link.clear()
bs_obj_out = get_html(source_url_path_out)
html_list_out = get_all_index(bs_obj_out)
for html_out in html_list_out:
...
# 引用不同的链接，下载不同的内容
download_directory_out = git_download_directory
source_url_path_out = git_url_path
source_link.clear()
bs_obj_out = get_html(source_url_path_out)
html_list_out = get_all_index(bs_obj_out)
for html_out in html_list_out:
